...
import ProductFbLiveUpdatePage from "../../components/ProductFbLiveUpdatePage";
  ...
  const handleUpdate = (data: ProductUpdateMutationResult) => {
    if (data.productUpdate.errors.length === 0) {
      notify({
        autohide: 3000,
        status: "success",
        text: `${data.productUpdate.product.name} бараа амжилттай хадгаллаа`
      });

      navigate(productAddFbLivePath);
    }
  };
    ...
    <>
      <WindowTitle
        title={maybe(
          () =>
            data.product.name +
            (createProductImageOpts.loading ? "loading..." : "")
        )}
      />
      <ProductFbLiveUpdatePage
